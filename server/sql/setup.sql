...
--
-- Scheduled events for sensor data cleaning
--
CREATE DEFINER=`root`@`localhost` EVENT `recent_data_cleanup` ON SCHEDULE EVERY 1 MINUTE STARTS '2016-02-26 00:00:00' ON COMPLETION PRESERVE ENABLE DO DELETE FROM `recent_data`
WHERE `timestamp`<DATE_SUB(NOW(), INTERVAL 1 HOUR);

CREATE DEFINER=`root`@`localhost` EVENT `hourly_data_cleanup` ON SCHEDULE EVERY 1 MINUTE STARTS '2016-02-26 00:00:00' ON COMPLETION PRESERVE ENABLE DO DELETE FROM `hourly_data`
WHERE `timestamp`<DATE_SUB(NOW(), INTERVAL 1 DAY);

CREATE DEFINER=`root`@`localhost` EVENT `daily_data_cleanup` ON SCHEDULE EVERY 1 MINUTE STARTS '2016-02-26 00:00:00' ON COMPLETION PRESERVE ENABLE DO DELETE FROM `daily_data`
WHERE `timestamp`<DATE_SUB(NOW(), INTERVAL 1 MONTH);

CREATE DEFINER=`root`@`localhost` EVENT `monthly_data_cleanup` ON SCHEDULE EVERY 1 MINUTE STARTS '2016-02-26 00:00:00' ON COMPLETION PRESERVE ENABLE DO DELETE FROM `monthly_data`
